...
def pagerank(index_file, edge_file, max_node_id, edge_count, damping_factor=0.85, iterations=10):
	...
	#size of long-long is 8 -> pairs = 2*8=16 -> total number of pairs=(max_node_id + 1)
	length=16 * (max_node_id + 1),
        access=mmap.ACCESS_READ)

    edge_map = mmap.mmap(
        edge_file.fileno(),
        #length=99999,  # ~~~ MODIFY THIS LINE (ii) ~~~
	#size of int is 4 -> pairs = 2*4=8 -> total number of pairs=edge_count
	length=8 * edge_count,
        access=mmap.ACCESS_READ)
