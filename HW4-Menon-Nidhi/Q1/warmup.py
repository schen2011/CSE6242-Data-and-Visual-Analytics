...
def write_data_to_binary_file(item_list, file_name):
    with open(file_name, "wb") as file_object:
		...
		#reference: https://docs.python.org/2/library/struct.html#struct-examples
		pack("<iq", item, item*item))
...
def parse_memory_map(file_map):
    ...
    '''
    #2 values -> first value:4 and second value: 8 -> 4 + 8 = 12
    for i in range(21):  # ~~~ MODIFY THIS LINE (iii) ~~~
        parsed_values.append(
            unpack("<iq", file_map[i * 12 : i * 12 + 12]))
...
def warmup():
    ...
    item_list = []
    for i in range(0,42):	# (0,42) since 1 inclusive
	if i%2 == 1:
	    item_list.append(i)
