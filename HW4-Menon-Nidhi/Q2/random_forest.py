...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        XX=np.array(XX)
        index=np.random.choice(XX.shape[0],n,replace=True)

        samples = XX[index,:-1] # sampled dataset
        labels = XX[index,-1]  # class labels for the sampled records
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                #pass
                OOB_tree = self.decision_trees[0]
                effective_vote = OOB_tree.classify(record)
