...
def entropy(class_y):
    ...
    total = zeroes + ones
    p_zeroes = float(zeroes)/total
    p_ones = float(ones)/total
    entropy = -(p_zeroes * math.log(p_zeroes,2) + p_ones * math.log(p_ones,2))
...
def information_gain(previous_y, current_y):
    ...
    info_gain = 0.45915
    """
    #print 'info gain', previous_y,current_y
    previous_entropy = entropy(previous_y)
    res = 0
    for i in range(len(current_y)):
        temp = entropy(current_y[i]) * (len(current_y[i]) / float(len(previous_y)))
        res += temp
    info_gain = previous_entropy - res
    #print 'in info gain',info_gain
    return info_gain
